...
def calculate():
    ...
    for i in range(values["months"]):
        ...
        values["total_inv"] = (
            values["total_inv"] / 100 * (100 + values["p_return"] / 12)
        )
...
def update_display_data():
    ...
    sv = st.slider(
        "Select month for detailed data and percentage changes compared to the"
        " previous month",
        min_value=1,
        max_value=i4,
        value=1,
    ...
    col1.metric(
        "Total Wealth",
        display_data[1].loc[sv, "Total Wealth"],
        delta(display_data[1], sv, "Total Wealth"),
    )
    col2.metric(
        "Total Profit",
        display_data[1].loc[sv, "Total Profit"],
        delta(display_data[1], sv, "Total Profit"),
    )
    col3.metric(
        "Monthly Profit",
        display_data[1].loc[sv, "Monthly Profit"],
