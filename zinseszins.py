...
i1 = st.number_input(
    "Enter value of current portfolio",
    min_value=0,
    value=1000,
    key="1")
i2 = st.number_input(
    ...
    key="2")
i3 = st.number_input(
    "Enter expected yearly return in percent",
    min_value=0,
    value=10,
    key="3")
i4 = st.number_input(
...
def calculate():
    ...
    for i in range(values["months"]):
        ...
        values["total_inv"] = values["total_inv"] / \
            100 * (100 + values["p_return"] / 12)
    ...
    chart_dict = {
        "Profit": arrays["profit_ar"],
        "Wealth": arrays["total_inv_ar"]}
    chart_data = pd.DataFrame(chart_dict, index=arrays["months_ar"])
    frame_dict = {
        "Total Wealth": arrays["total_inv_ar"],
        "Total Profit": arrays["profit_ar"],
        "Monthly Profit": arrays["monthly_profit_ar"]}
...
def delta(data, slider_value, column):
    if slider_value > 1:
        difference = data.loc[slider_value, column] - \
            data.loc[slider_value - 1, column]
...
def update_display_data():
    ...
    sv = st.slider(
        "Select month for detailed data and percentage changes compared to the previous month",
        min_value=1,
        max_value=i4,
        value=1,
        step=1)
    ...
    col1.metric("Total Wealth", display_data[1].loc[sv, "Total Wealth"], delta(
        display_data[1], sv, "Total Wealth"))
    col2.metric("Total Profit", display_data[1].loc[sv, "Total Profit"], delta(
        display_data[1], sv, "Total Profit"))
    col3.metric("Monthly Profit", display_data[1].loc[sv, "Monthly Profit"], delta(
        display_data[1], sv, "Monthly Profit"))
